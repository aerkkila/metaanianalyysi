...
def aja(luokitus,tunniste,vuosi):
    ...
    tight_layout()
    if '-s' in sys.argv:
        nimi = 'kuvia/vuojakaumalaatikko_vuosittain/%s_%s_%i.png' %(pripost[ppnum], tunniste, vuosi)
        savefig(nimi)
        clf()
...
def aja2(luokka,kausi):
    ...
    tight_layout()
    if '-s' in sys.argv:
        nimi = 'kuvia/vuojakaumalaatikko_vuosittain/%s_%s_%s.png' %(pripost[ppnum], luokka, kausi)
        savefig(nimi)
        clf()
        #os.system('gm convert -rotate 90 %s %s' %(nimi,nimi))
    else:
