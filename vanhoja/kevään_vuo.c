...
int main() {
    ...
    tallenna_kausi(tulokset, alut, loput, vuodet, alut_.pit/ikirluokkia);
...
#define K(a,b) if(write(fd, a, b) != (b)) warn("write rivillä %i", __LINE__);
    int fd = open("kev_met.bin", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    int pit = ikirluokkia*vuosia;
    int ik = ikirluokkia;
    K(&vuosia, 1);
    K(&ik, 1);
...
#undef K
    ...
    fd = open("kevät.bin", O_WRONLY | O_CREAT | O_TRUNC, 0644);
