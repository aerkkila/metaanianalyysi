...
/* Vuodata huomioidaan vain, jos hilaruudussa on ollut kaikki kaudet kaikkina vuosina.
   Muuten menee väärin, kun jaetaan vuosia-muuttujalla.
   Ikiroudan tapauksessa tilanne on vähän erilainen. */
...
void laske_ikir(struct laskenta* args) {
	...
	for(; t<aikapit; t++, tma.tm_mday++) {
		...
		if(lukitse_vuosien_määrä) args->kuluva_vuosi--; // tätä oli muutettu, joten palautetaan
		break;
	    }
	    if(apu==3) { // vuosi vaihtui
		if(lukitse_vuosien_määrä) args->kuluva_vuosi--;
		else hyväksy_data(args);
		lukitse_ikirvuosi = 0;
	    }
		...
		lukitse_vuosien_määrä = 1;
		continue;
	    }
	    lukitse_vuosien_määrä = 0;
	...
	/* Muunnetaan niin, että myöhemmin jaettaessa vuosilla jako menee oikein */
	for(int k=0; k<kausia; k++) {
	    args->ala_ja_aika[k] *= (double)vuosia / args->kuluva_vuosi;
	    args->ainemäärä[k]   *= (double)vuosia / args->kuluva_vuosi;
	}
    }
}
...
int main(int argc, char** argv) {
	...
	tm1         = (struct tm){.tm_year=2019-1900+1, .tm_mon=1-1, .tm_mday=15};
