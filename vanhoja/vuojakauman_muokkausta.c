...
/* Vähän makrotaikuutta / -pelleilyä */
#define aloita_jos(lista,i) do { char** __tmplista = lista; int __tmpi = i; if(0)
#define jos(arg) else if(!strcmp(__tmplista[__tmpi], #arg))
#define lopeta_jos } while(0)
...
const char* kausinimet[]     = {"whole_year", "summer", "freezing", "winter"};
const char* pripost_sisaan[] = {"flux_bio_prior", "flux_bio_posterior"};
const char* pripost_ulos[]   = {"pri", "post"};
enum                           {kopp_e, ikir_e, wetl_e};
const char*** luoknimet;

const char* kaudet[]     = {"summer", "freezing"};
int luokkabitit;
...
float     alat[LATPIT] = {0};
nct_vset *luok_wetl;
int       ppnum=1, kausia, aikapit;
float *restrict vuoptr;
char  *restrict kausiptr, *restrict luok_kopp, *restrict luok_ikir;
int pakota, verbose;
...
int argumentit(int argc, char** argv) {
	...
	aloita_jos(argv,i);
	jos(köpp) luokkabitit |= 1<<kopp_e;
	jos(ikir) luokkabitit |= 1<<ikir_e;
	jos(wetl) luokkabitit |= 1<<wetl_e;
	jos(pri)  ppnum    = 0;
	jos(post) ppnum    = 1;
	jos(-B)   pakota   = 1;
	jos(-v)   verbose  = 1;
...
static void* lue_ikir() {
    ...
    nct_free_vset(vset);
    return (luok_ikir = ret);
}
static void* lue_wetl() {
    return (luok_wetl = nct_read_ncfile("./BAWLD1x1.nc"));
}
int lue_luokitus() {
    static void* (*funktio[])(void) = { [kopp_e]=lue_kopp, [ikir_e]=lue_ikir, [wetl_e]=lue_wetl, };
    for(int i=0; i<3; i++)
	if(luokkabitit & (1<<i) && !funktio[i]())
	    return 1;
    return 0;
}

void nouda_data(int luokitusnum, int lajinum, float** vuoulos, float** cdf, int* kauden_pit) {
    FILE *f;

    if(pakota) goto TEE_DATA;
    for(int i=0; i<kausia; i++)
	if(access(aprintf("./vuojakaumadata/%s_%s_%s.bin", luoknimet[luokitusnumi][lajinum], kaudet[i], pripost_ulos[ppnum]), F_OK))
	    goto TEE_DATA;
...
int main(int argc, char** argv) {
    ...
    int lajeja = 0;
    if (luokkabitit & (1<<kopp_e)) lajeja += ARRPIT(koppnimet);
    if (luokkabitit & (1<<ikir_e)) lajeja += ARRPIT(ikirnimet);
    if (luokkabitit & (1<<wetl_e)) lajeja += ARRPIT(wetlnimet);
	...
	for(int i=0; i<kausia; i++) {
	    aloita_jos(kaudet, i);
	    jos(whole_year) kauden_kapasit[i] = (size_t)(aikapit*1.0 * resol*0.75);
	    jos(summer    ) kauden_kapasit[i] = (size_t)(aikapit*0.7 * resol*0.75);
	    jos(winter    ) kauden_kapasit[i] = (size_t)(aikapit*0.6 * resol*0.75);
	    jos(freezing  ) kauden_kapasit[i] = (size_t)(aikapit*0.1 * resol*0.75);
	    lopeta_jos;
	}
	...
	for(int i=0; i<3; i++)
	    if(luokkabitit & (1<<i))
		for(int lajinum=0; lajinum<ARRPIT; lajinum++)
		    nouda_data(i, lajinum, vuojako, cdf, kausi_pit+kausia*lajinum);
