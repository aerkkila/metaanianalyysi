...
def aja(ikirkö):
    ...
    if 1:
        ...
        luokitus_ = Dataset('köppen1x1maski.nc', 'r')['ET']
        l = np.ma.getdata(luokitus_[:]).flatten() * maski
        l = ~~l
        for ilk,lk in enumerate(luokat_):
            luokitus *=  l
    else:
    ...
    ax = subplot(1,1,1+ikirkö, projection=projektio)
    ax.coastlines()
