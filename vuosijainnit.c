...
float pisteen_vuo(const struct tiedot* restrict tiedot) {
    int n = 0;
    double summa = 0;
    for(int i=0; i<tiedot->vuosia; i++) {
	int alku  = montako_päivää(tiedot->aika0, tiedot->vuodet[i], tiedot->alut[i*tiedot->res]),
	    loppu = montako_päivää(tiedot->aika0, tiedot->vuodet[i], tiedot->loput[i*tiedot->res]);
	if(alku == VIRHE || loppu == VIRHE)
	    continue;
	for(int t=alku; t<loppu; t++)
	    summa += tiedot->vuo[t*tiedot->res] / tiedot->WET * tiedot->wet;
	n += loppu-alku;
    }
    return summa*1e9 / n;
}
...
char* luo_alue(const double* prfwet, const double* wet, char* alue, int xyres) {
	...
	alue[i] = alue[i] && wet[i] >= 0.05 && prfwet[i] < 0.97 && prfwet[i] > 0.03;
    return alue;
}
...
int main() {
    ...
    double *WET   = nct_load_data_with(bawvset, "wetland",            nc_get_var_double, sizeof(double)),
    ...
    int    *vuodt = nct_load_data_with(kauvset, "vuosi",              nc_get_var_int,    sizeof(int));
    ...
    char* alue = luo_alue(prfwet, WET, maski->data, xyres);
	...
	float *uusidata = malloc(xyres*sizeof(float));
        double *wet   = nct_load_data_with(bawvset, luokat[j], nc_get_var_double, sizeof(double));
	...
	for(int i=0; i<xyres; i++) {
	    ...
	    tiedot.vuo   = vuo+i;
	    tiedot.alut  = alut+i;
	    tiedot.loput = loput+i;
	    uusidata[i] = pisteen_vuo(&tiedot);
	}
