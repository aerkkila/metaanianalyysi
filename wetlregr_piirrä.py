...
def main():
    ...
    for i,nimi in enumerate(nimet):
        ...
        if tallenna:
            if i == 0:
                ax = axes()
                ax.scatter(wdata0, vuo, marker='.')
                xlabel('wetland')
                ylabel('flux')
                tight_layout()
                if nimet[0] != 'bog':
                    savefig('kuvia/vuopisteet_wetland_ikir_%s.png' %kaudet[i])
                    fraja = 35
                else:
                    savefig('kuvia/vuopisteet_wetland_%s.png' %kaudet[i])
                    fraja = 50
                clf()
            ax = axes()
            wraja = 0.05
            ...
            nimeä()
            tight_layout()
            savefig('kuvia/vuopisteet_%s_%s.png' %(nimi,kaudet[i]))
            continue
