 ...
 */
static void muunna(const ptyyppi* purettu, mtyyppi* muunnettu, const int* pohjoisrajat, size_t muunnettukoko, int *indeksit) {
    int muunn_ind = 0;
    int indind0   = 0;
    for(int j=0; j<alue.latpit; j++) {
	for(int i=0; i<alue.lonpit; i++) {
	    double summa = 0;
	    unsigned k = 0, jakaja = 0;
	    if (j <= pohjoisrajat[i]) {
		...
		for(k=0; indeksit[indind0+k] >= 0; k++) {
		    summa  +=   purettu[indeksit[indind0+k]];
		    jakaja += !!purettu[indeksit[indind0+k]];
		}
	    muunnettu[muunn_ind++] = summa / jakaja;
	    indind0 += vnta_maxpit;
	}
    }
}

static void muunna_nollitta(const ptyyppi* purettu, mtyyppi* muunnettu, size_t muunnettukoko, int *indeksit) {
    int muunn_ind = 0;
    int indind0   = 0;
    for(int j=0; j<alue.latpit; j++) {
	for(int i=0; i<alue.lonpit; i++) {
...
static void aseta_pohjoisrajat(const mtyyppi* muunnettu, int* rajat) {
    for(int j=0; j<alue.latpit; j++)
	for(int i=0; i<alue.lonpit; i++) {
	    mtyyppi luku = muunnettu[j*alue.lonpit+i];
	    if (0 < luku && luku < 90)
		rajat[i] = j;
	}
}
...
int main() {
    ...
    int nollien_pohjoisrajat[alue.lonpit];
    memset(nollien_pohjoisrajat, 0, sizeof(nollien_pohjoisrajat));
	...
	if (v == 0) {
	    muunna_nollitta(alkup, muunnettuptr, latlonres, indeksit);
	    aseta_pohjoisrajat(muunnettuptr, nollien_pohjoisrajat);
	}
	muunna(alkup, muunnettuptr, nollien_pohjoisrajat, latlonres, indeksit);
