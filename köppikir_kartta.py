...
luokat_köpp = ['C.b', 'D.a', 'D.b', 'D.c', 'D.d', 'ET', 'EF']
luokat_ikir = luokat.ikir[1:]
cmapnimi = "jet"
...
def piirrä_tarkasti(luokitus, cmap, ax, luokat_, värin_alku=0):
    dx = lon[1]-lon[0]
    dy = lat[1]-lat[0]
    for i,luokka in enumerate(luokat_):
        maski_2D = luokitus[i,:].reshape([len(lat),len(lon)])
        #Ei piirrä mitään, vaan on legend-funktiota varten.
        plot(-1,-1,'.',markersize=24,color=cmap(i+värin_alku),label=luokka,transform=platecarree)
