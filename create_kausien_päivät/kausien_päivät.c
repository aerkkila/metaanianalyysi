...
typedef char ft_tyyppi;

#define AA
#if defined AA
#define kelpaako(a, b) ((a) >= 0)
#define jäätyykö(lA, lB) ((lA) + 9*(int)(lB) >= 90)
#define talveako(lA, lB) ((lB) >= 90)
const char menetelmä_sy[] = "partly + 9*frozen >= 0.9";
const char menetelmä_ta[] = "frozen >= 0.9";
const char* tunniste = "";
const char* luettavaA = kansio "^partly_frozen_percent_[0-9]*.nc$";
const char* luettavaB = kansio "^frozen_percent_[0-9]*.nc$";
#else
virhe;
#endif
...
int talven_pituus(ft_tyyppi* lA, ft_tyyppi* lB, int tnyt, int pit_xy, int t_loppu, int vuosi) {
    kausimerkki = kesä;
...
int jäätym_pituus(ft_tyyppi* lA, ft_tyyppi* lB, int tnyt, int pit_xy, int t_loppu, int vuosi) {
    kausimerkki = talvi;
    int ajan_huippu = päiviä_taitteeseen(tnyt, vuosi+1);
    if (ajan_huippu <= 0)
...
int kesän_pituus(ft_tyyppi* lA, ft_tyyppi* lB, int tnyt, int pit_xy, int t_loppu, int vuosi) {
...
int main(int argc, char** argv) {
    ...
    ft_tyyppi* lA = nct_firstvar(luokkaA)->data;
    ft_tyyppi* lB = NULL;
    if (luokkaB)
